...
def rules_logic_pre_dealer(player, dealer, blackjack_multiplier):
    ...
    if player.best_outcome == 'Bust':
        ...
        print("No need for Dealer to go. Player loses with a {} multiplier".format(str(player.ROI)))
        dealer_to_go = False
    elif player.best_outcome == 'Blackjack' and dealer.cards[0].rank not in [1,10]:
        player.ROI = blackjack_multiplier
        print("Dealer has no chance to hit Blackjack. Player wins with a {} multiplier".format(str(blackjack_multiplier)))
        dealer_to_go = False
    else: 
        dealer_to_go = True
...
def rules_logic_post_dealer(player, dealer):
    ...
    if dealer.best_outcome == 'Bust':
        ...
        player.ROI = -1 * earning_multiplier
        print("Dealer has {} wheras Player has {}. Player loses with a {} multiplier".format(str(dealer.best_outcome), str(player.best_outcome), str(player.ROI)))
    elif int(dealer.best_outcome) < int(player.best_outcome):
        player.ROI = 1 * earning_multiplier
        print("Dealer has {} wheras Player has {}. Player wins with a {} multiplier".format(str(dealer.best_outcome), str(player.best_outcome), str(player.ROI)))
    else:
